...
Ide::Ide(BaseObjectType* cobject,
	...
	Gtk::Window(cobject), m_builder(builder), treePanel(builder,this),panelAcciones(builder,this){
	this->enlazarWidgets();
	this->maximize();
	this->vproyecto = NULL;
	this->show();
...
bool Ide::hayProyecto(){
	if (this->vproyecto==NULL){
		Gtk::MessageDialog mensaje(*this, "Error.", false, Gtk::MESSAGE_ERROR,
				Gtk::BUTTONS_OK);
		mensaje.set_secondary_text("No Hay proyecto cargado, operacion Imposible.");
		mensaje.run();
		return false;
	}
	return true;
