...
void AsistenteJerarquia::llenarComboBox(){
	std::vector<VistaEntidad *>::iterator it1 =  this->diagrama->vEntBegin();
	std::vector<VistaEntidad *>::iterator it2 =  this->diagrama->vEntEnd();
	while (it1 != it2){
...
void AsistenteJerarquia::on_ComboBox_click(){
	...
	if(iter)
	{
	  ...
	  VistaEntidad* omitir = row[this->m_ColumnasCombo.m_col_vEnt_Pointer];
	  this->llenarListaEntidades(omitir);
	}
}

void AsistenteJerarquia::on_botonAceptar_click() {
	VistaEntidad* entidadPadre = NULL;
