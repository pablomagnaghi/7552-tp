...
void VistaDiagrama::abrirXml(Diagrama* diagrama, const std::string& path,
...
void VistaDiagrama::obtenerComponentesModelo() {
	...
	std::vector<EntidadNueva*>::iterator itEntNueva;
	for (itEntNueva = diagrama->entidadesNuevasBegin();
			itEntNueva != diagrama->entidadesNuevasEnd(); ++itEntNueva) {
		this->componentesModelo.push_back((*itEntNueva));
		std::vector<Atributo*>::iterator itAtrib;
		for (itAtrib = (*itEntNueva)->atributosBegin(); itAtrib != (*itEntNueva)->atributosEnd();
				++itAtrib) {
			this->componentesModelo.push_back((*itAtrib));
			this->agregarAtributoCompuesto((*itAtrib));
		}
	...
	for (itRel = diagrama->relacionesBegin(); itRel != diagrama->relacionesEnd(); ++itRel) {
		...
		std::vector<UnionEntidadRelacion*>::iterator itUER;
		for (itUER = (*itRel)->unionesAEntidadBegin(); itUER != (*itRel)->unionesAEntidadEnd();
				++itUER)
	...
	for (itEntGlobal = diagrama->entidadesGlobalesBegin();
			itEntGlobal != diagrama->entidadesGlobalesEnd(); ++itEntGlobal)
